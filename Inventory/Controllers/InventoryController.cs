...
namespace Inventory.Controllers
{
    public class InventoryController : BaseController
    {
        ...
        public ActionResult Alert()
        {
            ...
            MySqlCommand command = mconn.CreateCommand();
            command.CommandText = "SELECT Inventory.InventoryId, Inventory.ItemId, Inventory.LocationId, "
                                    + "Inventory.Quantity, Items.LowLimit AS LowLimit,	Items.Name AS ItemName,	Locations.Name AS LocationName, "
            ...
            while (reader.Read())
            {
                inventories.Add(new ViewModels.InventoryIndex()
                {
                    ...
                    ItemLowLimit = reader.GetInt32("LowLimit")
                });
            }
            reader.Close();
           
            
            var sortedResults = inventories.OrderBy(c => c.Quantity);
